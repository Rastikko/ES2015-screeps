...
function builder(roomGame, creep) {
  ...
  if ((creepEmpty || transferEnergy) && spawnFinished) {
    ...
    for (var index in structures) {
      ...
      if (structure.structureType == 'rampart' && structure.hits < structure.hitsMax - 50) damagedRamparts.push(structure);
    }
    ...
    for (var index in halfBroken) {
      if (halfBroken[index].hits / halfBroken[index].hitsMax < 0.5) toRepair.push(halfBroken[index]);
    }if (toRepair.length) {
    ...
    if (targets) {
      ...
      if (!creep.pos.isNearTo(targets)) creep.moveTo(targets);
